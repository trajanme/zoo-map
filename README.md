...
| 2 | 園内マップ | `#/zoos/:zooId` | [Leaflet](https://leafletjs.com/) による実地図(航空写真タイル)背景の園内マップ。エリア・獣舎をポリゴンで色分けし、動物の位置に絵文字ピンを表示。ピンや獣舎をタップするとポップアップで動物名と詳細リンクを表示。現在地ボタン(📍)から自分の位置を表示することもできる |
...
園内マップ画面の右下に現在地ボタン(📍)があります。

- **タップして初めて** ブラウザの位置情報の利用許可を求めます(画面を開いただけでは要求しません)
- 許可すると `watchPosition` で位置を継続的に取得し、現在地を青いドット+精度円でマップ上に表示します
- **園の敷地から大きく外れている間はマーカーを表示しません**。ボタン脇の小さなステータス表示で
  「現在地を取得中…」「園外のため非表示」などの状態が分かるようになっており、正しく動作している
  ことが分かるようにしています
- ボタンを再タップすると現在地へ再センタリングします(園内にいる場合のみ)。ステータス表示内の
  「停止」で位置の追従を解除できます
- 許可を拒否した場合・ブラウザが Geolocation API に対応していない場合は、ボタンが無効状態になり、
  理由がステータス表示に出ます(エラーで画面が壊れることはありません)
- 園内マップ画面を離れると位置の監視(`watchPosition`)は自動的に解除されます

**プライバシー**: 取得した位置情報は、この現在地表示のためだけにブラウザ内(端末内)で使用し、
どこにも送信・保存しません。サーバーが存在しない静的サイトのため、送信先自体がありません。
